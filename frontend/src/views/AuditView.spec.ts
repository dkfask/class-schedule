...
describe('AuditView', () => {
  it('loads readable audit events and sends supported filters', async () => {
    ...
    const wrapper = mount(AuditView, { global: { stubs: { RouterLink: { props: ['to'], template: '<a :href="to"><slot /></a>' }, 'el-button': { template: '<button><slot /></button>' } } } })
    await flushPromises()
  ...
  it('points an empty audit trail back to the term overview', async () => {
    vi.stubGlobal('fetch', vi.fn(async (input: RequestInfo | URL) => {
      const url = String(input)
      if (url === '/api/terms') return response([{ code: '2026-FALL', name: '2026 秋季学期', status: 'ACTIVE' }])
      if (url.startsWith('/api/audit?')) return response({ items: [] })
      return response({})
    }))
    const wrapper = mount(AuditView, { global: { stubs: { RouterLink: { props: ['to'], template: '<a :href="to"><slot /></a>' }, 'el-button': { template: '<button><slot /></button>' } } } })
    await flushPromises()
    expect(wrapper.get('[data-testid="empty-state"]').text()).toContain('还没有操作记录')
