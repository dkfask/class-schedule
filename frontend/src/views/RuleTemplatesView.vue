...
<template>
  ...
  <section class="template-page" data-testid="rule-templates-page">
    <div class="template-grid">
      <section class="panel canvas-card template-list"><div class="panel-heading"><div><span class="eyebrow">SAVED BASELINES</span><h2>学校规则模板</h2></div><button class="quiet-button" :disabled="loading" @click="load">刷新</button></div><EmptyState v-if="!templates.length && !loading" title="还没有保存的规则模板" description="核对当前学期规则后，可以在右侧保存为一套可复用的学校规则。" action-label="查看规则检查" to="/rule-facts" /><button v-for="item in templates" :key="item.id" class="template-row" :class="{ selected: item.id === selectedId }" @click="selectedId = item.id"><strong>{{ item.name }} · v{{ item.version }}</strong><small>{{ item.code }} · {{ item.maintainedBy || '未填写维护人' }}</small><span>{{ item.description || '没有模板说明' }}</span></button></section>
