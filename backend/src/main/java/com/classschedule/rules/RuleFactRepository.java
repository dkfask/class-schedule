...
public class RuleFactRepository {
    ...
    public void upsertAvailability(String resourceType, AvailabilityRequest request) {
        ...
        jdbc.update("INSERT INTO " + table + " (" + resourceColumn + ", term_id, period_code, available) VALUES ((SELECT id FROM " + resourceTable + " WHERE code=?), (SELECT id FROM academic_term WHERE code=?), ?, ?) ON CONFLICT (" + resourceColumn + ", term_id, period_code) DO UPDATE SET available=EXCLUDED.available", request.resourceCode(), request.termCode(), request.periodCode(), request.available());
        audit("AVAILABILITY_UPSERT", resourceType + ":" + request.resourceCode() + ":" + request.periodCode());
    }

    @Transactional
    public void upsertRoomFeature(RoomFeatureRequest request) {
        requireActive("room", request.roomCode());
        jdbc.update("INSERT INTO room_feature_catalog(code,name) VALUES(?,?) ON CONFLICT(code) DO UPDATE SET name=EXCLUDED.name, active=TRUE", request.featureCode(), request.featureName() == null || request.featureName().isBlank() ? request.featureCode() : request.featureName());
        jdbc.update("INSERT INTO room_feature(room_id,feature_code) VALUES((SELECT id FROM room WHERE code=?),?) ON CONFLICT DO NOTHING", request.roomCode(), request.featureCode());
    ...
    public void addRequirementFeature(RequirementFeatureRequest request) {
        if (jdbc.queryForObject("SELECT COUNT(*) FROM teaching_requirement WHERE code=? AND active=TRUE", Integer.class, request.requirementCode()) == 0) throw new IllegalArgumentException("教学需求不存在: " + request.requirementCode());
        if (jdbc.queryForObject("SELECT COUNT(*) FROM room_feature_catalog WHERE code=? AND active=TRUE", Integer.class, request.featureCode()) == 0) throw new IllegalArgumentException("教室特征不存在或已停用: " + request.featureCode());
        jdbc.update("INSERT INTO teaching_requirement_feature(teaching_requirement_id,feature_code) VALUES((SELECT id FROM teaching_requirement WHERE code=?),?) ON CONFLICT DO NOTHING", request.requirementCode(), request.featureCode());
        audit("REQUIREMENT_FEATURE_UPSERT", request.requirementCode() + ":" + request.featureCode());
    }
    ...
    public record AvailabilityItem(String resourceType, String resourceCode, String periodCode, boolean available) {}
    ...
    public void deleteAvailability(String resourceType, AvailabilityRequest request) {
        ...
        jdbc.update("DELETE FROM " + table + " WHERE " + resourceColumn + " = (SELECT id FROM " + resourceTable + " WHERE code = ?) AND term_id = (SELECT id FROM academic_term WHERE code = ?) AND period_code = ?", request.resourceCode(), request.termCode(), request.periodCode());
        audit("AVAILABILITY_DELETE", resourceType + ":" + request.resourceCode() + ":" + request.periodCode());
    }
    ...
    public record ActivityGroupItem(String code, String name, String activityType, List<String> requirementCodes) {}

    public List<FeatureItem> listFeatureCatalog() {
        return jdbc.query("SELECT code, name, active FROM room_feature_catalog ORDER BY code", (rs, row) -> new FeatureItem(rs.getString("code"), rs.getString("name"), rs.getBoolean("active")));
    }

    public List<RoomFeatureItem> listRoomFeatures(String roomCode) {
        String sql = "SELECT r.code AS room_code, rf.feature_code, c.name FROM room_feature rf JOIN room r ON r.id=rf.room_id JOIN room_feature_catalog c ON c.code=rf.feature_code ORDER BY r.code, rf.feature_code";
        if (roomCode == null || roomCode.isBlank()) {
            return jdbc.query(sql, (rs, row) -> new RoomFeatureItem(rs.getString("room_code"), rs.getString("feature_code"), rs.getString("name")));
        }
        return jdbc.query(sql.replace(" ORDER BY", " WHERE r.code=? ORDER BY"), (rs, row) -> new RoomFeatureItem(rs.getString("room_code"), rs.getString("feature_code"), rs.getString("name")), roomCode);
    }
    ...
    public List<RequirementFeatureItem> listRequirementFeatures(String requirementCode) {
        String sql = "SELECT r.code, f.feature_code FROM teaching_requirement_feature f JOIN teaching_requirement r ON r.id=f.teaching_requirement_id ORDER BY r.code, f.feature_code";
        if (requirementCode == null || requirementCode.isBlank()) {
            return jdbc.query(sql, (rs, row) -> new RequirementFeatureItem(rs.getString("code"), rs.getString("feature_code")));
        }
        return jdbc.query(sql.replace(" ORDER BY", " WHERE r.code=? ORDER BY"), (rs, row) -> new RequirementFeatureItem(rs.getString("code"), rs.getString("feature_code")), requirementCode);
    }
    ...
    public List<AvailabilityItem> listAvailability(String termCode) {
        ...
        items.addAll(jdbc.query("SELECT r.code, a.period_code, a.available FROM teacher_availability a JOIN teacher r ON r.id=a.teacher_id WHERE a.term_id=? ORDER BY r.code, a.period_code",
                (rs, row) -> new AvailabilityItem("TEACHER", rs.getString("code"), rs.getString("period_code"), rs.getBoolean("available")), termId));
        items.addAll(jdbc.query("SELECT r.code, a.period_code, a.available FROM room_availability a JOIN room r ON r.id=a.room_id WHERE a.term_id=? ORDER BY r.code, a.period_code",
                (rs, row) -> new AvailabilityItem("ROOM", rs.getString("code"), rs.getString("period_code"), rs.getBoolean("available")), termId));
        items.addAll(jdbc.query("SELECT r.code, a.period_code, a.available FROM student_group_availability a JOIN student_group r ON r.id=a.student_group_id WHERE a.term_id=? ORDER BY r.code, a.period_code",
                (rs, row) -> new AvailabilityItem("STUDENT_GROUP", rs.getString("code"), rs.getString("period_code"), rs.getBoolean("available")), termId));
        return items;
    }

    private void audit(String action, String detail) {
        jdbc.update("INSERT INTO audit_event(action,aggregate_type,aggregate_id,actor,actor_user_id,actor_kind,outcome,detail) VALUES(?, 'RULE_FACT', ?, 'system', (SELECT id FROM app_user WHERE username='system'), 'SERVICE', 'SUCCESS', jsonb_build_object('detail', ?))", action, detail, detail);
    }

    private void requireActive(String table, String code) {
        if (jdbc.queryForObject("SELECT COUNT(*) FROM " + table + " WHERE code=? AND active=TRUE", Integer.class, code) == 0) throw new IllegalArgumentException("资源不存在或已停用: " + code);
    }

    private void requireTermPeriod(String termCode, String periodCode) {
        if (jdbc.queryForObject("SELECT COUNT(*) FROM period_template p JOIN academic_term t ON t.id=p.term_id WHERE t.code=? AND p.code=?", Integer.class, termCode, periodCode) == 0) throw new IllegalArgumentException("节次不属于目标学期: " + periodCode);
    }
}
