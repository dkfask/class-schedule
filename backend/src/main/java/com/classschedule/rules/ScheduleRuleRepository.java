...
public class ScheduleRuleRepository {
    ...
    public ScheduleRuleRepository(JdbcTemplate jdbc, AcademicTermResolver terms) {
        this.jdbc = jdbc;
        this.terms = terms;
    }
    ...
    public void upsert(ScheduleRuleRequest request) {
        ...
        String persistedScopeCode =
                "TERM".equals(scopeType) ? "__TERM__" : blankToNull(request.scopeCode());
        ...
        if (!List.of("HARD", "MEDIUM", "SOFT").contains(severity))
            throw new IllegalArgumentException("不支持的规则级别");
        Long profile =
                jdbc.queryForObject(
                        "INSERT INTO schedule_rule_profile(term_id,code,name) VALUES(?,'DEFAULT','默认规则配置') ON CONFLICT(term_id,code) DO UPDATE SET active=TRUE RETURNING id",
                        Long.class,
                        termId);
        jdbc.update(
                ...
                scopeType,
                persistedScopeCode,
                request.normalizedIntValue() == 0 ? null : request.normalizedIntValue(),
    ...
    private void validateScopeResource(String scopeType, String scopeCode, long termId) {
        ...
        String sql =
                "TEACHING_REQUIREMENT".equals(scopeType)
    ...
    public void delete(long id) {
        if (jdbc.update("UPDATE schedule_rule_instance SET active=FALSE WHERE id=?", id) != 1)
