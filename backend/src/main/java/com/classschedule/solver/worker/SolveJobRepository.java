...
public class SolveJobRepository {
    ...
    public SolveJobRepository(
            JdbcTemplate jdbc,
            ScheduleSnapshotHashService snapshots,
            SolveReadinessService readiness,
    ...
    @Transactional
    public SolveJobHandle enqueue(
            String idempotencyKey, String submittedByUsername, String termCode) {
        ...
        if (idempotencyKey != null && !idempotencyKey.isBlank()) {
            ...
            String ownerClause =
                    " AND (u.username = CAST(? AS varchar) OR CAST(? AS varchar) = 'system')";
            Object[] ownerArgs = new Object[] {owner, owner};
            List<Object> params = new java.util.ArrayList<>();
            params.add(idempotencyKey);
            params.addAll(java.util.Arrays.asList(ownerArgs));
            List<SolveJobHandle> existing =
                    jdbc.query(
                            "SELECT j.id, j.schedule_version_id, j.status FROM solve_job j LEFT JOIN app_user u ON u.id = j.submitted_by_user_id WHERE j.idempotency_key = ? AND j.status IN ('QUEUED','RUNNING')"
                                    + ownerClause,
                            (rs, rowNum) ->
                                    new SolveJobHandle(
                                            rs.getLong("id"),
    ...
    private void lockIdempotencyKey(String idempotencyKey, String submittedByUsername) {
        ...
        jdbc.queryForRowSet(
                        ...
                        owner.length() + ":" + owner + ":" + idempotencyKey)
                .next();
    }
    ...
    public int recoverExpired() {
        int reclaimed =
                jdbc.update(
                        "UPDATE solve_job SET status = 'QUEUED', worker_id = NULL, lease_until = NULL, heartbeat_at = NULL, next_attempt_at = CURRENT_TIMESTAMP WHERE status = 'RUNNING' AND lease_until IS NOT NULL AND lease_until < CURRENT_TIMESTAMP AND cancel_requested = FALSE AND deadline_at > CURRENT_TIMESTAMP");
        List<long[]> expired =
                jdbc.query(
                        "SELECT id, schedule_version_id FROM solve_job WHERE status = 'RUNNING' AND lease_until IS NOT NULL AND lease_until < CURRENT_TIMESTAMP AND cancel_requested = FALSE AND deadline_at <= CURRENT_TIMESTAMP",
                        (rs, rowNum) ->
                                new long[] {rs.getLong("id"), rs.getLong("schedule_version_id")});
        int failed = 0;
        for (long[] job : expired) {
            jdbc.update(
                    "UPDATE schedule_version SET status = 'FAILED' WHERE id = ? AND status = 'SOLVING'",
                    job[1]);
            int updated =
                    jdbc.update(
                            "UPDATE solve_job SET status = 'FAILED', error_code = 'DEADLINE_EXCEEDED', error_message = '求解任务超过截止时间', finished_at = CURRENT_TIMESTAMP, lease_until = NULL WHERE id = ? AND status = 'RUNNING' AND cancel_requested = FALSE",
                            job[0]);
            if (updated == 1) {
                failed++;
                jdbc.update(
                        "INSERT INTO audit_event (action, aggregate_type, aggregate_id, detail) VALUES ('SOLVE_DEADLINE', 'SOLVE_JOB', ?, jsonb_build_object('errorCode', 'DEADLINE_EXCEEDED'))",
                        String.valueOf(job[0]));
            }
        }
    ...
    public void heartbeat(long jobId, String workerId, int progress) {
        jdbc.update(
                "UPDATE solve_job SET heartbeat_at = CURRENT_TIMESTAMP, lease_until = CURRENT_TIMESTAMP + INTERVAL '30 seconds', progress = ? WHERE id = ? AND worker_id = ? AND status = 'RUNNING'",
                progress,
                jobId,
                workerId);
    }
    ...
    public boolean complete(long jobId, long versionId, String score, Timetable solution) {
        if (jdbc.update(
                        "UPDATE solve_job SET status = 'COMPLETING' WHERE id = ? AND status = 'RUNNING' AND cancel_requested = FALSE",
                        jobId)
                != 1) return false;
        jdbc.update("DELETE FROM schedule_assignment WHERE schedule_version_id = ?", versionId);
        solution.getOccurrences()
                .forEach(
                        occurrence -> {
                            ...
                            jdbc.update(
                                    ...
                                    room == null
                                            ? new String[0]
                                            : room.getFeatures().toArray(String[]::new),
    ...
    private boolean identityComplete(Timetable solution) {
        return solution.getOccurrences().stream()
                .allMatch(
                        item ->
    ...
    public void fail(long jobId, long versionId, String code, String message) {
        Boolean cancelled =
                jdbc.queryForObject(
                        "SELECT cancel_requested FROM solve_job WHERE id = ? AND status = 'RUNNING'",
                        Boolean.class,
                        jobId);
        ...
        Integer attempt =
                jdbc.queryForObject(
                        "SELECT attempt FROM solve_job WHERE id = ? AND status = 'RUNNING'",
                        Integer.class,
                        jobId);
        Integer maxAttempts =
                jdbc.queryForObject(
                        "SELECT max_attempts FROM solve_job WHERE id = ?", Integer.class, jobId);
        Boolean withinDeadline =
                jdbc.queryForObject(
                        "SELECT deadline_at > CURRENT_TIMESTAMP FROM solve_job WHERE id = ?",
                        Boolean.class,
                        jobId);
        if (attempt != null
                && maxAttempts != null
                && attempt < maxAttempts
                && !Boolean.FALSE.equals(withinDeadline)) {
            jdbc.update(
                    "UPDATE solve_job SET status = 'QUEUED', error_code = ?, error_message = ?, worker_id = NULL, lease_until = NULL, heartbeat_at = NULL, next_attempt_at = CURRENT_TIMESTAMP + (? * INTERVAL '2 seconds') WHERE id = ? AND status = 'RUNNING' AND cancel_requested = FALSE",
                    code,
                    message,
                    attempt,
                    jobId);
        ...
        jdbc.update(
                ...
                versionId);
        jdbc.update(
                "UPDATE solve_job SET status = 'FAILED', error_code = ?, error_message = ?, finished_at = CURRENT_TIMESTAMP, lease_until = NULL WHERE id = ? AND status = 'RUNNING' AND cancel_requested = FALSE",
                code,
                message,
                jobId);
        jdbc.update(
                "INSERT INTO audit_event (action, aggregate_type, aggregate_id, actor, detail) VALUES ('SOLVE_FAILED', 'SOLVE_JOB', ?, 'worker', jsonb_build_object('errorCode', ?, 'errorMessage', ?))",
    ...
    public SolveJobDetails details(long jobId, String requesterUsername) {
        String ownerClause =
                requesterUsername == null
                        ? ""
                        : " AND (u.username = ? AND u.enabled = TRUE OR ? = 'system' OR EXISTS (SELECT 1 FROM app_user_role ur JOIN app_role r ON r.id=ur.role_id JOIN app_user admin ON admin.id=ur.user_id WHERE admin.username=? AND admin.enabled=TRUE AND r.code='USER_ADMIN' AND r.active=TRUE))";
        Object[] args =
                requesterUsername == null
                        ? new Object[] {jobId}
                        : new Object[] {
                            jobId, requesterUsername, requesterUsername, requesterUsername
                        };
        try {
    ...
    private SolveJobDetails mapDetails(ResultSet rs) throws SQLException {
        return new SolveJobDetails(
                ...
                stringTime(rs.getObject("deadline_at", OffsetDateTime.class)));
    }

    private String stringTime(OffsetDateTime time) {
