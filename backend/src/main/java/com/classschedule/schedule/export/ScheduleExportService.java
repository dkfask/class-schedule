...
public class ScheduleExportService {
    ...
    public List<ScheduleAssignmentView> assignments(
    ...
    public byte[] xlsx(long versionId, String view, String resourceCode) {
        ...
        try (SXSSFWorkbook workbook = new SXSSFWorkbook();
            ...
            title.createCell(0)
                    .setCellValue("课表版本 v" + version.id() + " / revision " + version.revision());
            title.createCell(1).setCellValue("生成时间");
            title.createCell(2).setCellValue(OffsetDateTime.now().toString());
            Row header = sheet.createRow(2);
            String[] columns = {
                "occurrenceKey", "课程编码", "课程名称", "教师", "班级", "节次", "教室", "来源", "锁定", "连续节数"
            };
            ...
            for (ScheduleAssignmentView item : version.assignments()) {
                ...
                String[] values = {
                    ...
                    item.roomCode(),
                    item.source(),
                    String.valueOf(item.locked()),
                    String.valueOf(item.duration())
                };
