...
public class ValidationReportService {
    ...
    public Report validate(long versionId) {
        ...
        List<Violation> violations =
                validator.validate(versionId, version).stream()
                        .map(
                                item ->
                                        new Violation(
                                                item.code(),
    ...
    public byte[] xlsx(long versionId) {
        ...
        try (SXSSFWorkbook workbook = new SXSSFWorkbook();
            ...
            title.createCell(0)
                    .setCellValue(
                            "课表版本 v" + report.versionId() + " / revision " + report.revision());
            title.createCell(1).setCellValue("生成时间");
            title.createCell(2).setCellValue(OffsetDateTime.now().toString());
            Row header = sheet.createRow(2);
            String[] columns = {
                "code", "severity", "weight", "blocking", "message", "resourceCode", "occurrenceKey"
            };
            ...
            for (Violation violation : report.violations()) {
                ...
                String[] values = {
                    violation.code(),
                    violation.severity(),
                    String.valueOf(violation.weight()),
                    String.valueOf(violation.blocking()),
                    violation.message(),
                    violation.resourceCode(),
                    violation.occurrenceKey()
                };
    ...
    public record Report(long versionId, long revision, String status, List<Violation> violations) {
        public boolean valid() {
            return violations.stream().noneMatch(Violation::blocking);
        }
    }
