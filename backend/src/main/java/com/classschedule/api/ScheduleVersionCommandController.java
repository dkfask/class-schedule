...
public class ScheduleVersionCommandController {
    ...
    public ResponseEntity<Map<String, Object>> fork(
        ...
        try {
            long newVersionId =
                    repository.fork(versionId, request.name(), authentication.getName());
    ...
    public ResponseEntity<?> lock(
            @PathVariable long versionId,
            @Valid @RequestBody VersionLockRequest request,
            Authentication authentication) {
        String actor = authentication.getName();
        try {
            repository.lockVersion(
                    versionId, actor, request.normalizedReason(), request.expectedRevision());
    ...
    public ResponseEntity<?> undo(
        ...
        try {
            ...
            ScheduleVersionRepositoryModels.MutationResult result =
                    repository.undo(versionId, groupId, null, actor, key);
            return mutationResponse(versionId, result, "UNDONE");
        } catch (VersionMutationException exception) {
            return conflict(exception);
        } catch (IllegalArgumentException exception) {
            return ResponseEntity.status(HttpStatus.CONFLICT)
                    .body(Map.of("status", "UNDO_REJECTED", "message", exception.getMessage()));
        }
    }
        ...
        try {
            String key =
                    idempotencyKey != null
                            ...
                            : request == null ? null : request.normalizedKey();
            ScheduleVersionRepositoryModels.MutationResult result =
                    repository.redo(versionId, groupId, null, actor, key);
            return mutationResponse(versionId, result, "REDONE");
        } catch (VersionMutationException exception) {
            return conflict(exception);
        } catch (IllegalArgumentException exception) {
            return ResponseEntity.status(HttpStatus.CONFLICT)
                    .body(Map.of("status", "REDO_REJECTED", "message", exception.getMessage()));
        }
    }

    @PostMapping("/{versionId}/adjustments/exchange-candidates")
    public ResponseEntity<?> exchangeCandidates(
            @PathVariable long versionId,
            @Valid @RequestBody ExchangeCandidatesRequest request,
            Authentication authentication) {
        try {
            if (!repository.canAccessVersion(versionId, authentication.getName()))
                return ResponseEntity.notFound().build();
    ...
    public ResponseEntity<?> exchange(
        ...
        try {
            ScheduleVersionRepositoryModels.MutationResult result =
                    repository.exchange(
                            versionId, request, authentication.getName(), idempotencyKey);
            return mutationResponse(versionId, result, "EXCHANGED");
        } catch (VersionMutationException exception) {
            return conflict(exception);
        } catch (IllegalArgumentException exception) {
            return ResponseEntity.status(HttpStatus.CONFLICT)
                    .body(Map.of("status", "EXCHANGE_REJECTED", "message", exception.getMessage()));
        }
    }
