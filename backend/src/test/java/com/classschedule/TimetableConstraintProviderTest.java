...
class TimetableConstraintProviderTest {
    private final ConstraintVerifier<TimetableConstraintProvider, Timetable> verifier =
            ConstraintVerifier.build(
                    new TimetableConstraintProvider(), Timetable.class, LessonOccurrence.class);
    ...
    void durationOccupiesAdjacentPeriods() {
        ...
        Timetable overlap =
                new Timetable(
                        List.of(first, second), List.of(room, otherRoom), List.of(left, right));
    ...
    void availabilityUsesExactWeekdayPeriodCodesForDuration() {
        ...
        occurrence.setAvailablePeriodCodes(
                new java.util.LinkedHashSet<>(List.of("MON-1", "MON-2", "TUE-2")));
        assertThat(new TimetableConstraintProvider().resourceAvailabilityForTest(occurrence))
                .isFalse();
        occurrence.setAvailablePeriodCodes(
                new java.util.LinkedHashSet<>(List.of("MON-1", "TUE-2")));
        assertThat(new TimetableConstraintProvider().resourceAvailabilityForTest(occurrence))
                .isTrue();
    }
    ...
    void joinedMembersAtSameIndexMayShareTeacherAndRoom() {
        ...
        verifier.verifyThat(TimetableConstraintProvider::teacherConflict)
                .given(left, right)
                .penalizesBy(0);
        verifier.verifyThat(TimetableConstraintProvider::roomConflictForTest)
                .given(left, right)
                .penalizesBy(0);
    }
    ...
    void typedDailyMaxPenalizesByWeightOnMediumLayer() {
        TypedScheduleRule rule =
                new TypedScheduleRule(
                        "TEACHER_DAILY_MAX", "TERM", "__TERM__", 1, null, "MEDIUM", 5);
        Timeslot first = new Timeslot("MON-1", 1, 1, "周一 第1节");
        Timeslot second = new Timeslot("MON-2", 1, 2, "周一 第2节");
        Room room = new Room("A101", "教学楼 A101", 50);
        LessonOccurrence left = occurrence(1L, "T001", "G7-1");
    ...
    void typedDailyMaxDoesNotAggregateDifferentStudentGroupsForOneSubject() {
        TypedScheduleRule rule =
                new TypedScheduleRule(
                        "SUBJECT_DAILY_MAX", "TERM", "__TERM__", 1, null, "MEDIUM", 5);
        Timeslot first = new Timeslot("MON-1", 1, 1, "周一 第1节");
        Room room = new Room("A101", "教学楼 A101", 50);
        LessonOccurrence left = occurrence(1L, "T001", "G7-1");
        LessonOccurrence right = occurrence(2L, "T002", "G7-2");
    ...
    void typedDailyMaxPenalizesEachExcessPeriod() {
        TypedScheduleRule rule =
                new TypedScheduleRule("TEACHER_DAILY_MAX", "TERM", "__TERM__", 1, null, "SOFT", 5);
        Timeslot first = new Timeslot("MON-1", 1, 1, "周一 第1节");
        Timeslot second = new Timeslot("MON-2", 1, 2, "周一 第2节");
        Timeslot third = new Timeslot("MON-3", 1, 3, "周一 第3节");
        Room room = new Room("A101", "教学楼 A101", 50);
    ...
    void typedSpreadPenalizesEachMissingDay() {
        TypedScheduleRule rule =
                new TypedScheduleRule(
                        "SUBJECT_MIN_SPREAD_DAYS", "TERM", "__TERM__", 4, null, "SOFT", 3);
        Timeslot first = new Timeslot("MON-1", 1, 1, "周一 第1节");
        Timeslot second = new Timeslot("MON-2", 1, 2, "周一 第2节");
        Room room = new Room("A101", "教学楼 A101", 50);
        LessonOccurrence left = occurrence(1L, "T001", "G7-1");
    ...
    void pinnedOccurrenceKeepsItsPlanningValueDuringSolve() {
        ...
        LessonOccurrence solvedPinned =
                solved.getOccurrences().stream()
