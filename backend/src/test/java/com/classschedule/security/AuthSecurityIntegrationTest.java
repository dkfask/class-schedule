...
class AuthSecurityIntegrationTest {
    ...
    void seedUsers() {
        ...
        jdbc.update("INSERT INTO app_user(username,password_hash,display_name,enabled) VALUES(?,?,?,TRUE) ON CONFLICT (username) DO UPDATE SET password_hash=EXCLUDED.password_hash, display_name=EXCLUDED.display_name, enabled=TRUE", "auth-planner", encoder.encode("planner-pass"), "测试排课员");
        jdbc.update("INSERT INTO app_user(username,password_hash,display_name,enabled) VALUES(?,?,?,TRUE) ON CONFLICT (username) DO UPDATE SET password_hash=EXCLUDED.password_hash, display_name=EXCLUDED.display_name, enabled=TRUE", "auth-viewer", encoder.encode("viewer-pass"), "测试只读用户");
        jdbc.update("INSERT INTO app_user(username,password_hash,display_name,enabled) VALUES(?,?,?,TRUE) ON CONFLICT (username) DO UPDATE SET password_hash=EXCLUDED.password_hash, display_name=EXCLUDED.display_name, enabled=TRUE", "auth-admin", encoder.encode("admin-pass"), "测试管理员");
        jdbc.update("INSERT INTO app_user(username,password_hash,display_name,enabled) VALUES(?,?,?,FALSE) ON CONFLICT (username) DO UPDATE SET password_hash=EXCLUDED.password_hash, display_name=EXCLUDED.display_name, enabled=FALSE", "disabled-owner", encoder.encode("disabled-pass"), "停用排课员");
        ...
        jdbc.update("INSERT INTO app_user_role(user_id,role_id) SELECT u.id,r.id FROM app_user u,app_role r WHERE u.username='auth-admin' AND r.code IN ('PLANNER','USER_ADMIN')");
    }
    ...
    void loginRotatesAnExistingSessionId() throws Exception {
        ...
        var result = mockMvc.perform(post("/api/auth/login")
                        ...
                        .content("{\"username\":\"auth-planner\",\"password\":\"planner-pass\"}"))
        ...
        mockMvc.perform(get("/api/auth/me").session(authenticatedSession)).andExpect(status().isOk());
    }
    ...
    void plannerWithViewerRoleRetainsPlannerAccess() throws Exception {
        long archived = seedVersion("ARCHIVED");
        mockMvc.perform(get("/api/schedule-versions/" + archived).with(user("auth-planner").roles("PLANNER", "VIEWER")))
                .andExpect(status().isOk());
    }
    ...
    private long seedVersion(String status, String owner) {
        ...
        Long scenarioId = jdbc.queryForObject("INSERT INTO schedule_scenario(term_id,owner_user_id,name) VALUES(?,(SELECT id FROM app_user WHERE username=?),?) RETURNING id", Long.class, termId, owner, "security-" + status + "-" + System.nanoTime());
        return jdbc.queryForObject("INSERT INTO schedule_version(scenario_id,owner_user_id,status,score,legacy_identity_unverified) VALUES(?,(SELECT id FROM app_user WHERE username=?),?,?,FALSE) RETURNING id", Long.class, scenarioId, owner, status, "0hard/0soft");
    }
    ...
    void createdVersionIsOwnedBySubmittingPlanner() throws Exception {
        String created = mockMvc.perform(post("/api/solve-jobs").with(csrf()).contentType(MediaType.APPLICATION_JSON)
                        .content("{\"idempotencyKey\":\"auth-owned-version-" + System.nanoTime() + "\"}"))
                .andExpect(status().isOk()).andReturn().getResponse().getContentAsString();
        var body = new com.fasterxml.jackson.databind.ObjectMapper().readTree(created);
        long versionId = body.get("versionId").asLong();

        mockMvc.perform(get("/api/schedule-versions/" + versionId).with(user("auth-planner").roles("PLANNER")))
                .andExpect(status().isOk());
        mockMvc.perform(get("/api/schedule-versions/" + versionId).with(user("other-planner").roles("PLANNER")))
                .andExpect(status().isNotFound());
        assertThat(jdbc.queryForObject("SELECT owner_user_id = (SELECT id FROM app_user WHERE username='auth-planner') FROM schedule_version WHERE id=?", Boolean.class, versionId)).isTrue();
        assertThat(jdbc.queryForObject("SELECT s.owner_user_id = v.owner_user_id FROM schedule_version v JOIN schedule_scenario s ON s.id=v.scenario_id WHERE v.id=?", Boolean.class, versionId)).isTrue();
    }
    ...
    void anotherPlannerCannotReadOrModifyAnOwnedVersion() throws Exception {
        ...
        mockMvc.perform(get("/api/schedule-versions/" + versionId)
                ...
                .andExpect(status().isNotFound());
        mockMvc.perform(post("/api/schedule-versions/" + versionId + "/adjustments/1")
                        .with(user("other-planner").roles("PLANNER"))
                        .with(csrf()).contentType(MediaType.APPLICATION_JSON)
                        .content("{\"timeslotCode\":\"MON-1\",\"roomCode\":\"A101\",\"reason\":\"越权调整\"}"))
    ...
    void diffCannotUseAnotherUsersVersionAsComparisonBaseline() throws Exception {
        ...
        mockMvc.perform(get("/api/schedule-versions/" + ownVersionId + "/diff?againstVersionId=" + otherVersionId))
                .andExpect(status().isNotFound());
    }

    @Test
    @WithMockUser(username = "auth-admin", roles = {"PLANNER", "USER_ADMIN"})
    void userAdminCanReadAndForkAnotherUsersVersion() throws Exception {
        ...
        mockMvc.perform(get("/api/schedule-versions/" + sourceVersionId))
                .andExpect(status().isOk());
        String response = mockMvc.perform(post("/api/schedule-versions/" + sourceVersionId + "/fork")
                        .with(csrf()).contentType(MediaType.APPLICATION_JSON)
                        .content("{\"name\":\"管理员分支\"}"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.status").value("DRAFT"))
                .andReturn().getResponse().getContentAsString();
        long forkedVersionId = new com.fasterxml.jackson.databind.ObjectMapper().readTree(response).get("versionId").asLong();
        assertThat(jdbc.queryForObject("SELECT v.owner_user_id = (SELECT id FROM app_user WHERE username='auth-admin') AND s.owner_user_id = v.owner_user_id FROM schedule_version v JOIN schedule_scenario s ON s.id=v.scenario_id WHERE v.id=?", Boolean.class, forkedVersionId)).isTrue();
    }
    ...
    void viewerCannotReadAnUnpublishedVersionOwnedByAnotherUser() throws Exception {
        long versionId = seedVersion("CANDIDATE");
        mockMvc.perform(get("/api/schedule-versions/" + versionId)).andExpect(status().isForbidden());
        mockMvc.perform(get("/api/schedule-versions/" + versionId + "/filtered?view=CLASS")).andExpect(status().isForbidden());
    }
    ...
    void disabledOwnerCannotReadItsPreviouslyOwnedJobOrVersion() throws Exception {
        String created = mockMvc.perform(post("/api/solve-jobs").with(csrf()).contentType(MediaType.APPLICATION_JSON)
                        .content("{\"idempotencyKey\":\"disabled-owner-job-" + System.nanoTime() + "\"}"))
                .andExpect(status().isOk()).andReturn().getResponse().getContentAsString();
        var body = new com.fasterxml.jackson.databind.ObjectMapper().readTree(created);
        ...
        mockMvc.perform(get("/api/solve-jobs/" + jobId).with(user("auth-planner").roles("PLANNER")))
                .andExpect(status().isNotFound());
        mockMvc.perform(post("/api/solve-jobs/" + jobId + "/cancel").with(user("auth-planner").roles("PLANNER")).with(csrf()))
                .andExpect(status().isNotFound());
        mockMvc.perform(get("/api/schedule-versions/" + versionId).with(user("auth-planner").roles("PLANNER")))
                .andExpect(status().isNotFound());
    }
    ...
    void unknownVersionAndJobIdsHaveStableNotFoundResponses() throws Exception {
        ...
        mockMvc.perform(get("/api/schedule-versions/" + missingId)).andExpect(status().isNotFound());
        mockMvc.perform(get("/api/schedule-versions/" + missingId + "/filtered?view=CLASS")).andExpect(status().isNotFound());
        mockMvc.perform(get("/api/solve-jobs/" + missingId)).andExpect(status().isNotFound()).andExpect(jsonPath("$.code").value("JOB_NOT_FOUND"));
        mockMvc.perform(post("/api/solve-jobs/" + missingId + "/cancel").with(csrf())).andExpect(status().isNotFound()).andExpect(jsonPath("$.code").value("JOB_NOT_FOUND"));
    }
    ...
    void concurrentSolveJobSubmissionsReturnTheSameHandle() throws Exception {
        ...
        try {
            var first = executor.submit(() -> {
                start.await();
                return mockMvc.perform(post("/api/solve-jobs")
                                .with(user("auth-planner").roles("PLANNER"))
                                .with(csrf())
                                .contentType(MediaType.APPLICATION_JSON)
            ...
            assertThat(secondBody.get("versionId").asLong()).isEqualTo(firstBody.get("versionId").asLong());
            assertThat(jdbc.queryForObject(
                    ...
                    Integer.class, "auth-planner", key)).isEqualTo(1);
        } finally {
