...
class ScheduleCommandIntegrationTest {
    ...
    static final PostgreSQLContainer<?> POSTGRES =
            new PostgreSQLContainer<>("postgres:16-alpine")
                    .withDatabaseName("class_schedule_command")
    ...
    void seedOwners() {
        jdbc.update(
                "INSERT INTO app_user(username,password_hash,display_name) VALUES('test-planner','{noop}test','测试排课员'),('alice','{noop}test','Alice'),('bob','{noop}test','Bob') ON CONFLICT (username) DO NOTHING");
    }
    ...
    void adjustmentCreatesOneGroupAndIsIdempotent() throws Exception {
        ...
        String response =
                mockMvc.perform(
                        ...
                        .andExpect(jsonPath("$.commandIds").isArray())
        ...
        mockMvc.perform(
                        post("/api/schedule-versions/" + version + "/adjustments/1")
                                ...
                                .content(body))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.groupId").value(first.get("groupId").asText()))
                .andExpect(jsonPath("$.revision").value(1));
        assertThat(
                        jdbc.queryForObject(
                                "SELECT COUNT(*) FROM adjustment_command_group WHERE schedule_version_id=?",
                                Integer.class,
                                version))
                .isEqualTo(1);
        assertThat(
                        jdbc.queryForObject(
                                "SELECT COUNT(*) FROM adjustment_command WHERE schedule_version_id=?",
                                Integer.class,
                                version))
                .isEqualTo(1);
    }
    ...
    void revisionConflictDoesNotWriteCommand() throws Exception {
        ...
        mockMvc.perform(
                ...
                .andExpect(jsonPath("$.code").value("VERSION_REVISION_CONFLICT"))
                .andExpect(jsonPath("$.currentRevision").value(0));
        assertThat(
                        jdbc.queryForObject(
                                "SELECT COUNT(*) FROM adjustment_command_group", Integer.class))
                .isZero();
    }
    ...
    void undoAndRedoRestoreAssignmentAsAnAtomicGroup() throws Exception {
        ...
        String response =
                mockMvc.perform(
                                post("/api/schedule-versions/" + version + "/adjustments/1")
                                        .header("Idempotency-Key", "undo-source")
                                        .with(csrf())
                                        .contentType(MediaType.APPLICATION_JSON)
                                        .content(body))
                        .andReturn()
        ...
        mockMvc.perform(
                        post("/api/schedule-versions/"
                                        ...
                                        + groupId
                                        + "/undo")
                                .header("Idempotency-Key", "undo-event")
                                .with(csrf())
                                .contentType(MediaType.APPLICATION_JSON)
                                .content("{}"))
                .andExpect(status().isOk())
        ...
        mockMvc.perform(
                        post("/api/schedule-versions/"
                                        ...
                                        + groupId
                                        + "/redo")
                                .header("Idempotency-Key", "redo-event")
                                .with(csrf())
                                .contentType(MediaType.APPLICATION_JSON)
                                .content("{}"))
                .andExpect(status().isOk())
    ...
    void lockBlocksOtherOwnerAndUnlockRestoresEditing() throws Exception {
        ...
        mockMvc.perform(
                        post("/api/schedule-versions/" + version + "/lock")
                                .with(user("alice").roles("PLANNER"))
                                .with(csrf())
                                .contentType(MediaType.APPLICATION_JSON)
                                .content(
                                        objectMapper.writeValueAsString(
                                                Map.of("owner", "alice", "reason", "审核中"))))
                .andExpect(status().isOk());
        String body =
                objectMapper.writeValueAsString(
                        Map.of(
        ...
        mockMvc.perform(
                        delete("/api/schedule-versions/" + version + "/lock")
                                .with(user("alice").roles("PLANNER"))
                                .with(csrf())
    ...
    void lockOwnerCanPublishButOtherOwnerCannot() throws Exception {
        ...
        mockMvc.perform(
                        post("/api/schedule-versions/" + version + "/lock")
                                ...
                                .content(
                                        objectMapper.writeValueAsString(
                                                Map.of("owner", "alice", "reason", "发布审核"))))
                .andExpect(status().isOk());
        mockMvc.perform(
    ...
    void commandHistoryReturnsGroupedChildren() throws Exception {
        ...
        String body =
                objectMapper.writeValueAsString(
                        Map.of("timeslotCode", "TUE-1", "roomCode", "A101", "reason", "历史测试"));
        mockMvc.perform(
                        post("/api/schedule-versions/" + version + "/adjustments/1")
                                .with(csrf())
                                .contentType(MediaType.APPLICATION_JSON)
                                .content(body))
                .andExpect(status().isOk());
        mockMvc.perform(get("/api/schedule-versions/" + version + "/adjustments/commands"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$[0].commandType").value("ADJUST"))
    ...
    private long seedCandidateVersion(String owner) {
        ...
        Long scenario =
                jdbc.queryForObject(
                        "INSERT INTO schedule_scenario(term_id,owner_user_id,name) VALUES(?,(SELECT id FROM app_user WHERE username=?), 'command-test') RETURNING id",
                        Long.class,
                        term,
                        owner);
        Long version =
                jdbc.queryForObject(
                        "INSERT INTO schedule_version(scenario_id,owner_user_id,status,score,legacy_identity_unverified) VALUES(?,(SELECT id FROM app_user WHERE username=?), 'CANDIDATE','0hard/0soft',FALSE) RETURNING id",
                        Long.class,
                        scenario,
                        owner);
        jdbc.update(
                "INSERT INTO schedule_assignment(schedule_version_id,occurrence_id,occurrence_key,subject_code,subject_name,teacher_code,teacher_name,student_group_code,student_group_name,timeslot_code,timeslot_label,weekday,period_no,room_code,room_name,source,locked,duration,student_count,required_features,room_features,room_capacity) VALUES(?,1,'1','MATH','数学','T001','张老师','G7-1','七年级1班','MON-1','周一 第1节',1,1,'A101','教学楼 A101','SOLVER',false,1,0,'{}','{}',50)",
                version);
        jdbc.update(
                "INSERT INTO schedule_assignment(schedule_version_id,occurrence_id,occurrence_key,subject_code,subject_name,teacher_code,teacher_name,student_group_code,student_group_name,timeslot_code,timeslot_label,weekday,period_no,room_code,room_name,source,locked,duration,student_count,required_features,room_features,room_capacity) VALUES(?,2,'2','CHN','语文','T001','张老师','G7-2','七年级2班','MON-2','周一 第2节',1,2,'A102','教学楼 A102','SOLVER',false,1,0,'{}','{}',50)",
                version);
        return version;
    }
}
