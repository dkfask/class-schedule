...
class ScheduleExportIntegrationTest {
    ...
    static final PostgreSQLContainer<?> POSTGRES =
            new PostgreSQLContainer<>("postgres:16-alpine")
                    .withDatabaseName("class_schedule_export")
    ...
    void seedOwner() {
        jdbc.update(
                "INSERT INTO app_user(username,password_hash,display_name) VALUES('export-planner','{noop}test','测试排课员') ON CONFLICT (username) DO NOTHING");
    }
    ...
    void exportsXlsxPdfValidationAndPrintData() throws Exception {
        ...
        try (var workbook = WorkbookFactory.create(new ByteArrayInputStream(xlsx))) {
            ...
            assertThat(workbook.getSheet("课表").getRow(2).getCell(0).getStringCellValue())
                    .isEqualTo("occurrenceKey");
        }
        byte[] pdf =
                mockMvc.perform(
                                get(
        ...
        try (org.apache.pdfbox.pdmodel.PDDocument document =
            ...
            assertThat(extracted)
                    .contains("v" + version, "revision 0", "T001", "G7-1", "MON-1", "A101");
        ...
        mockMvc.perform(get("/api/schedule-versions/" + version + "/print?view=CLASS"))
                ...
                .andExpect(content().string(org.hamcrest.Matchers.containsString("课表版本")));
    }
    ...
    private long seedCandidateVersion() {
        ...
        Long scenario =
                jdbc.queryForObject(
                        "INSERT INTO schedule_scenario(term_id,owner_user_id,name) VALUES(?,(SELECT id FROM app_user WHERE username='export-planner'), 'export-test') RETURNING id",
                        Long.class,
                        term);
        Long version =
                jdbc.queryForObject(
                        "INSERT INTO schedule_version(scenario_id,owner_user_id,status,score,legacy_identity_unverified) VALUES(?,(SELECT id FROM app_user WHERE username='export-planner'), 'CANDIDATE','0hard/0soft',FALSE) RETURNING id",
                        Long.class,
                        scenario);
        jdbc.update(
                "INSERT INTO schedule_assignment(schedule_version_id,occurrence_id,occurrence_key,subject_code,subject_name,teacher_code,teacher_name,student_group_code,student_group_name,timeslot_code,timeslot_label,weekday,period_no,room_code,room_name,source,locked,duration,student_count,required_features,room_features,room_capacity) VALUES(?,1,'export-1','MATH','数学','T001','张老师','G7-1','七年级1班','MON-1','周一 第1节',1,1,'A101','教学楼 A101','SOLVER',false,1,0,'{}','{}',50)",
                version);
        return version;
    }
}
