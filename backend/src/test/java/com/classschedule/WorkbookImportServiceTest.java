...
class WorkbookImportServiceTest {
    ...
    void masterDataAllowsMissingOptionalSheets() throws Exception {
        ...
        try (var workbook = new XSSFWorkbook(); var output = new ByteArrayOutputStream()) {
            ...
            sheet(workbook, "教师", MasterDataSchemaRegistry.headers("教师").toArray(String[]::new), new String[] {"T1", "教师", "TRUE"});
            sheet(workbook, "班级", MasterDataSchemaRegistry.headers("班级").toArray(String[]::new), new String[] {"G1", "班级", "HOMEROOM", "30", "TRUE"});
            sheet(workbook, "课程", MasterDataSchemaRegistry.headers("课程").toArray(String[]::new), new String[] {"S1", "课程", "TRUE"});
            sheet(workbook, "教学需求", MasterDataSchemaRegistry.headers("教学需求").toArray(String[]::new), new String[] {"R1", "2026-FALL", "G1", "S1", "T1", "1", "1", "30", "", "TRUE"});
            workbook.write(output);
            var preview = new WorkbookImportService(jdbc).preview(multipart(output, "minimal.xlsx"));
    ...
    void masterDataStillRequiresCoreSheets() throws Exception {
        ...
        try (var workbook = new XSSFWorkbook(); var output = new ByteArrayOutputStream()) {
            ...
            sheet(workbook, "教学需求", MasterDataSchemaRegistry.headers("教学需求").toArray(String[]::new), new String[] {"R1", "2026-FALL", "G1", "S1", "T1", "1", "1", "30", "", "TRUE"});
            workbook.write(output);
            var preview = new WorkbookImportService(jdbc).preview(multipart(output, "missing-core.xlsx"));
            assertThat(preview.status()).isEqualTo("INVALID");
            assertThat(preview.issues()).anyMatch(issue -> issue.code().equals("MISSING_SHEET") && issue.sheet().equals("班级"));
        }
    }
    ...
    private JdbcTemplate mockJdbc() {
        ...
        when(jdbc.query(anyString(), any(RowMapper.class), any(Object[].class))).thenReturn(java.util.List.of("ACTIVE"));
