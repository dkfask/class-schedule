...
class SolverBenchmarkTest {
    ...
    void solvesARepresentativeLargeTimetableWithinConfiguredTermination() {
        long terminationMillis =
                Long.parseLong(System.getProperty("solver.benchmark.termination-ms", "3000"));
        ...
        assertThat(solved.getOccurrences())
                .hasSize(240)
                .allSatisfy(
                        item -> {
                            assertThat(item.getTimeslot())
                                    .as("timeslot for occurrence %s", item.getId())
                                    .isNotNull();
                            assertThat(item.getRoom())
                                    .as("room for occurrence %s", item.getId())
                                    .isNotNull();
                        });
        if (Boolean.parseBoolean(
                System.getProperty("solver.benchmark.require-zero-hard", "false"))) {
            assertThat(solved.getScore().hardScore())
                    .as("hard score after %d ms", terminationMillis)
                    .isZero();
        }
        System.out.printf(
                "solver benchmark: occurrences=%d, terminationMs=%d, elapsedMs=%d, score=%s%n",
                solved.getOccurrences().size(),
                terminationMillis,
                elapsedMillis,
                solved.getScore());
    }
        ...
        for (int index = 0; index < 240; index++) {
            ...
            LessonOccurrence occurrence =
                    new LessonOccurrence(
                            ...
                            "SUB" + subjectNumber,
                            "课程 " + subjectNumber,
                            "T" + String.format("%02d", index % 60),
                            "教师 " + (index % 60),
                            "G" + String.format("%02d", index % 24),
                            "班级 " + (index % 24));
            ...
            if (subjectNumber == 0)
                occurrence.setRequiredFeatures(new LinkedHashSet<>(List.of("LAB")));
        ...
        List<TypedScheduleRule> rules =
                List.of(
                        new TypedScheduleRule(
                                "TEACHER_DAILY_MAX",
                                "TERM",
                                TypedScheduleRule.TERM_SCOPE_SENTINEL,
                                3,
                                null,
                                "HARD",
                                1),
