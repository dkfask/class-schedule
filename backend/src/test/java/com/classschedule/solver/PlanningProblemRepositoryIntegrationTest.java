...
class PlanningProblemRepositoryIntegrationTest {
    ...
    void loadsTypedRulesIntoPlanningProblemFacts() {
        jdbc.update("INSERT INTO schedule_rule_profile(term_id,code,name) VALUES((SELECT id FROM academic_term WHERE code='2026-FALL'),'PLANNING-TEST','规划测试')");
        long profileId = jdbc.queryForObject("SELECT id FROM schedule_rule_profile WHERE code='PLANNING-TEST'", Long.class);
        try {
            jdbc.update("INSERT INTO schedule_rule_instance(profile_id,rule_code,scope_type,scope_code,int_value,severity,weight) VALUES(?,'TEACHER_DAILY_MAX','TERM','__TERM__',4,'MEDIUM',3)", profileId);
            PlanningProblem problem = repository.loadDefault();
            assertThat(problem.rules()).anyMatch(rule -> "TEACHER_DAILY_MAX".equals(rule.ruleCode())
                    && rule.limit() == 4 && rule.weight() == 3 && rule.isMedium());
        } finally {
    ...
    void loadsDurationAndPinnedPeriodIntoOccurrences() {
        ...
        try {
            PlanningProblem problem = repository.loadDefault();
            LessonOccurrence occurrence = problem.occurrences().stream()
    ...
    void loadsRoomAndRequirementFactsForTheWholeProblem() {
        jdbc.update("INSERT INTO room_feature_catalog(code,name) VALUES('BATCH_FEATURE','批量特征') ON CONFLICT (code) DO NOTHING");
        ...
        Long requirementId = jdbc.queryForObject(
                ...
                Long.class);
        jdbc.update("INSERT INTO room_feature(room_id,feature_code) VALUES(?, 'BATCH_FEATURE')", roomId);
        jdbc.update("INSERT INTO room_availability(room_id,term_id,period_code,available) VALUES(?,(SELECT id FROM academic_term WHERE code='2026-FALL'),'MON-1',FALSE)", roomId);
        jdbc.update("INSERT INTO teaching_requirement_feature(teaching_requirement_id,feature_code) VALUES(?, 'BATCH_FEATURE')", requirementId);

        try {
            PlanningProblem problem = repository.loadDefault();
            Room room = problem.rooms().stream().filter(item -> "BATCH-ROOM".equals(item.getId())).findFirst().orElseThrow();
            LessonOccurrence occurrence = problem.occurrences().stream()
                    .filter(item -> item.getId().equals(requirementId * 100))
